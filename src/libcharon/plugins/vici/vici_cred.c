	...
	private_vici_cred_t *this, char *name, u_int id, vici_message_t *message)
{
	...
	hex = message->get_str(message, NULL, "keyid");
	if (!hex)
	{
	...
	{	/* provide the pin in a temporary credential set to access the key */
		shared = shared_key_create(SHARED_PIN, chunk_from_str(pin));
		owner = identification_create_from_encoding(ID_KEY_ID, keyid);
		set = mem_cred_create();
	...
	if (shared && unique)
	{	/* use the CKA_ID as owner, but the key identifier as unique ID */
		owner = identification_create_from_encoding(ID_KEY_ID, keyid);
